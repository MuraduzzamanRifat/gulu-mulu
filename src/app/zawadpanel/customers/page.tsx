...
function hrefFor(params: { q?: string; page?: number }): string {
  ...
  return query ? `/zawadpanel/customers?${query}` : '/zawadpanel/customers'
}
...
export default async function AdminCustomersPage({ searchParams }: PageProps) {
  ...
  return (
    <>
      ...
      <Card className="p-3">
        <form method="get" action="/zawadpanel/customers" className="flex flex-col gap-2 sm:flex-row">
      ...
      <Card className="mt-4">
        {customers.length === 0 ? (
          <EmptyState
            ...
            action={
              filtered ? (
                <Link href="/zawadpanel/customers" className={buttonVariants({ variant: 'outline' })}>
                  Clear search
                </Link>
              ) : null
            }
          ...
          <>
            ...
            <ul className="divide-y divide-line lg:hidden">
              {customers.map((c) => (
                <li key={c.id}>
                  <Link
                    href={`/zawadpanel/customers/${c.id}`}
