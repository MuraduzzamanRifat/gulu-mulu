...
function hrefFor(params: HrefParams): string {
  ...
  return query ? `/zawadpanel/orders?${query}` : '/zawadpanel/orders'
}
      ...
      <Card className="mt-4">
        {orders.length === 0 ? (
          <EmptyState
            ...
            action={
              filtered ? (
                <Link href="/zawadpanel/orders" className={buttonVariants({ variant: 'outline' })}>
                  Clear filters
                </Link>
              ) : null
            }
          ...
          <>
            ...
            <ul className="divide-y divide-line lg:hidden">
              {orders.map((order) => (
                <li key={order.id}>
                  <Link
                    href={`/zawadpanel/orders/${order.id}`}
