...
function hrefFor(params: HrefParams): string {
  ...
  return query ? `/zawadpanel/products?${query}` : '/zawadpanel/products'
}
...
export default async function AdminProductsPage({ searchParams }: PageProps) {
  ...
  return (
    <>
      ...
      <Card className="p-3">
        <form method="get" action="/zawadpanel/products" className="flex flex-col gap-2 sm:flex-row">
      ...
      <Card className="mt-4">
        {products.length === 0 ? (
          <EmptyState
            ...
            action={
              filtered ? (
                <Link href="/zawadpanel/products" className={buttonVariants({ variant: 'outline' })}>
                  Clear filters
                </Link>
              ) : null
            }
